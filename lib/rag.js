    ...
    try {
        ...
        return {
            ...
            chunks: chunks.map((c, i) => ({
                ...c,
                id: `${id}-${i}`,
                contentLower: c.content.toLowerCase() // Pre-compute lowercase for faster search
            })),
            indexed: new Date().toISOString()
        };
    } catch (error) {
        // Log error but don't fail entire index operation
...
export async function ragIndexDirectory(params) {
    ...
    async function walkDir(dir, depth = 0) {
        ...
        try {
            ...
            console.debug(`[RAG] Skipping directory ${dir}: ${e.message}`);
        }
    }
