...
function calculateSimilarity(queryWords, textLower) {
    let matches = 0;
    // Optimization: Early exit if no query words
...
async function indexFileAsync(filePath) {
    try {
        // Check file size first to avoid reading massive files
        const stats = await fs.promises.stat(filePath);
        if (stats.size > 10 * 1024 * 1024) { // 10MB limit
            console.warn(`[RAG] Skipping large file: ${filePath} (${Math.round(stats.size / 1024 / 1024)}MB)`);
            return null;
        }
        ...
        return {
            ...
            // Store less content in memory for the main doc object, full content is in chunks if needed
            // Actually, we keep content for display, but maybe limit it more
...
export async function ragIndexDirectory(params) {
    const {
        ...
        maxFiles = 100,
        excludePatterns = ['node_modules', '.git', 'dist', 'build', '__pycache__', 'coverage', '.next', '.nuxt']
    } = params;
    ...
    const CONCURRENCY = 5; // Reduced concurrency to be nicer to FS and CPU
    const indexed = [];
