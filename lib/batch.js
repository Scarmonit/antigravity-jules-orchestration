...
export class BatchProcessor {
    constructor(julesRequest, createSession) {
        ...
        this.cleanupInterval = setInterval(() => this.cleanupBatches(), 3600000);
    }

    /**
     * Clean up old batches to prevent memory leaks
     */
    cleanupBatches(maxAgeMs = 24 * 60 * 60 * 1000) { // Default 24 hours
        const now = Date.now();
        let removed = 0;
    ...
    async createBatch(tasks, options = {}) {
        // Run cleanup occasionally on creation too, just in case
        if (Math.random() < 0.05) this.cleanupBatches();
