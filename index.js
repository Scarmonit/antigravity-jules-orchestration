...
import { getIssue, getIssuesByLabel, formatIssueForPrompt, validateGitHubParams } from './lib/github.js';
...
import { julesRequest } from './lib/jules-client.js';
import rateLimitMiddleware from './middleware/rateLimit.js';
import { initializeToolRegistry, getToolHandler, getToolRegistrySize } from './lib/tool-registry.js';
import {
  storeSessionOutcome,
...
app.get('/mcp/tools', cacheMiddleware, (req, res) => {
  // Use a static list or dynamically generate from registry if we had metadata
  // For now, returning the same comprehensive list as before
  res.json({
    tools: [
      // Original tools
      {
...
async function getAnalytics(days = 7) {
  ...
  // LRU cache doesn't always expose stats depending on version
  const cacheStats = typeof apiCache.info === 'function' ? apiCache.info() : { size: apiCache.size };
  ...
  return {
    ...
    queue: sessionQueue.stats(), cache: cacheStats
  };
}
