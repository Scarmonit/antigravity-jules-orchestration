...
import { julesRequest, createJulesSession, retryWithBackoff, circuitBreaker } from './lib/jules-client.js';
...
const rateLimiter = new RedisRateLimiter({
  redis: {
    ...
    connectTimeout: 2000 // Quick timeout to failover to memory
  },
  failover: {
    strategy: 'fail-closed', // Use local cache on Redis failure
    localCacheSize: 5000
  },
  tiers: {
    free: { requestsPerMinute: 100 }, // Matching original limit
    pro: { requestsPerMinute: 1000 },
    enterprise: { requestsPerMinute: 100000 }
  }
});

// Initialize Rate Limiter
rateLimiter.initialize().catch(err => {
  console.warn('Rate limiter initialization warning (running in failover mode):', err.message);
});

// Apply rate limiter to MCP routes
...
function initializeToolRegistry() {
  // Jules API tools
  toolRegistry.register('jules_list_sources', (p) => julesRequest('GET', '/sources'));
  toolRegistry.register('jules_create_session', (p) => createJulesSession(p, invalidateCaches));
  toolRegistry.register('jules_list_sessions', (p) => julesRequest('GET', '/sessions'));
  toolRegistry.register('jules_get_session', (p) => julesRequest('GET', '/sessions/' + p.sessionId));
  toolRegistry.register('jules_send_message', async (p) => {
    const result = await julesRequest('POST', '/sessions/' + p.sessionId + ':sendMessage', { message: p.message });
    invalidateCaches();
    return result;
  });
  toolRegistry.register('jules_approve_plan', async (p) => {
    const result = await julesRequest('POST', '/sessions/' + p.sessionId + ':approvePlan', {});
    invalidateCaches();
    return result;
  });
  toolRegistry.register('jules_get_activities', (p) => julesRequest('GET', '/sessions/' + p.sessionId + '/activities'));

  // GitHub Issue Integration
  toolRegistry.register('jules_create_from_issue', (p) => createSessionFromIssue(p));
  toolRegistry.register('jules_batch_from_labels', (p) => createSessionsFromLabel(p));

  // Batch Processing
  toolRegistry.register('jules_batch_create', (p) => batchProcessor.createBatch(p.tasks, { parallel: p.parallel }));
  toolRegistry.register('jules_batch_status', (p) => batchProcessor.getBatchStatus(p.batchId));
  toolRegistry.register('jules_batch_approve_all', (p) => batchProcessor.approveAllInBatch(p.batchId));

  // Monitoring
  toolRegistry.register('jules_monitor_all', (p) => sessionMonitor.monitorAll());
  toolRegistry.register('jules_session_timeline', (p) => sessionMonitor.getSessionTimeline(p.sessionId));

  // Ollama Local LLM
  toolRegistry.register('ollama_list_models', (p) => listOllamaModels());
  toolRegistry.register('ollama_completion', (p) => ollamaCompletion(p));
  toolRegistry.register('ollama_code_generation', (p) => ollamaCodeGeneration(p));
  toolRegistry.register('ollama_chat', (p) => ollamaChat(p));

  // RAG Tools
  toolRegistry.register('ollama_rag_index', (p) => ragIndexDirectory(p));
  toolRegistry.register('ollama_rag_query', (p) => ragQuery(p));
  toolRegistry.register('ollama_rag_status', (p) => ragStatus());
  toolRegistry.register('ollama_rag_clear', (p) => ragClear());

  // v2.5.0: Session Management
  toolRegistry.register('jules_cancel_session', (p) => cancelSession(p.sessionId));
  toolRegistry.register('jules_retry_session', (p) => retrySession(p.sessionId, p.modifiedPrompt));
  toolRegistry.register('jules_get_diff', (p) => getSessionDiff(p.sessionId));
  toolRegistry.register('jules_list_batches', () => batchProcessor.listBatches());
  toolRegistry.register('jules_delete_session', (p) => deleteSession(p.sessionId));
  toolRegistry.register('jules_clear_cache', () => { apiCache.clear(); return { success: true, message: 'Cache cleared' }; });
  toolRegistry.register('jules_cache_stats', () => ({ ...apiCache.stats(), circuitBreaker: { failures: circuitBreaker.failures, isOpen: circuitBreaker.isOpen() } }));
  toolRegistry.register('jules_cancel_all_active', (p) => cancelAllActiveSessions(p.confirm));

  // v2.5.0: Session Templates
  toolRegistry.register('jules_create_template', (p) => createTemplate(p.name, p.description, p.config));
  toolRegistry.register('jules_list_templates', () => listTemplates());
  toolRegistry.register('jules_create_from_template', (p) => createFromTemplate(p.templateName, p.overrides));
  toolRegistry.register('jules_delete_template', (p) => deleteTemplate(p.name));

  // v2.5.0: Session Cloning & Search
  toolRegistry.register('jules_clone_session', (p) => cloneSession(p.sessionId, p.modifiedPrompt, p.newTitle));
  toolRegistry.register('jules_search_sessions', (p) => searchSessions(p.query, p.state, p.limit));

  // v2.5.0: PR Integration
  toolRegistry.register('jules_get_pr_status', (p) => getPrStatus(p.sessionId));
  toolRegistry.register('jules_merge_pr', (p) => mergePr(p.owner, p.repo, p.prNumber, p.mergeMethod));
  toolRegistry.register('jules_add_pr_comment', (p) => addPrComment(p.owner, p.repo, p.prNumber, p.comment));

  // v2.5.0: Session Queue
  toolRegistry.register('jules_queue_session', (p) => ({ success: true, item: sessionQueue.add(p.config, p.priority) }));
  toolRegistry.register('jules_get_queue', () => ({ queue: sessionQueue.list(), stats: sessionQueue.stats() }));
  toolRegistry.register('jules_process_queue', () => processQueue());
  toolRegistry.register('jules_clear_queue', () => ({ success: true, cleared: sessionQueue.clear() }));

  // v2.5.0: Batch Retry & Analytics
  toolRegistry.register('jules_batch_retry_failed', (p) => batchRetryFailed(p.batchId));
  toolRegistry.register('jules_get_analytics', (p) => getAnalytics(p.days));

  // v2.5.2: Semantic Memory Integration
  toolRegistry.register('memory_recall_context', (p) => recallContextForTask(p.task, p.repository));
  toolRegistry.register('memory_store', (p) => storeManualMemory(p));
  toolRegistry.register('memory_search', (p) => searchMemories(p));
  toolRegistry.register('memory_related', (p) => getRelatedMemories(p.memoryId, p.limit));
  toolRegistry.register('memory_reinforce', (p) => reinforceSuccessfulPattern(p.memoryId, p.boost));
  toolRegistry.register('memory_forget', (p) => decayOldMemories(p.olderThanDays, p.belowImportance));
  toolRegistry.register('memory_health', () => checkMemoryHealth().then(healthy => ({ healthy, url: process.env.SEMANTIC_MEMORY_URL || 'not configured' })));
  toolRegistry.register('memory_maintenance_schedule', () => getMemoryMaintenanceSchedule());

  // v2.6.0: Render Integration for Auto-Fix
  toolRegistry.register('render_connect', (p) => renderConnect(p.apiKey, p.webhookSecret));
  toolRegistry.register('render_disconnect', () => renderDisconnect());
  toolRegistry.register('render_status', () => ({ configured: isRenderConfigured(), autoFix: getRenderAutoFixStatus() }));
  toolRegistry.register('render_list_services', () => renderListServices());
  toolRegistry.register('render_list_deploys', (p) => renderListDeploys(p.serviceId, p.limit));
  toolRegistry.register('render_get_build_logs', (p) => renderGetBuildLogs(p.serviceId, p.deployId));
  toolRegistry.register('render_analyze_failure', async (p) => {
  ...
  toolRegistry.register('render_autofix_status', () => getRenderAutoFixStatus());
  toolRegistry.register('render_set_autofix', (p) => setRenderAutoFixEnabled(p.enabled));
  toolRegistry.register('render_add_monitored_service', (p) => addRenderMonitoredService(p.serviceId));
  toolRegistry.register('render_remove_monitored_service', (p) => removeRenderMonitoredService(p.serviceId));
  toolRegistry.register('render_trigger_autofix', async (p) => {
  ...
  // v2.6.0: Suggested Tasks
  toolRegistry.register('jules_suggested_tasks', (p) => getSuggestedTasks(p.directory, {
    types: p.types,
    minPriority: p.minPriority,
    limit: p.limit,
    includeGitInfo: p.includeGitInfo
  }));
  toolRegistry.register('jules_fix_suggested_task', async (p) => {
  ...
  toolRegistry.register('jules_clear_suggested_cache', () => clearSuggestedTasksCache());
}
...
app.post('/mcp/execute', validateRequest(mcpExecuteSchema), async (req, res) => {
  ...
  structuredLog('info', 'Executing tool', { tool, parameters });

  try {
    const result = await handler(parameters);
    structuredLog('info', 'Tool completed successfully', { tool });
    res.json({ success: true, result });
  } catch (error) {
    structuredLog('error', 'Tool failed', { tool, error: error.message });
...
async function createFromTemplate(templateName, overrides = {}) {
  ...
  template.usageCount++;
  return await createJulesSession({ ...template.config, ...overrides }, invalidateCaches);
}
...
async function processQueue() {
  ...
  try {
    const session = await createJulesSession(next.config, invalidateCaches);
...
const server = app.listen(PORT, '0.0.0.0', () => {
  ...
  batchProcessor = new BatchProcessor(julesRequest, (c) => createJulesSession(c, invalidateCaches));
