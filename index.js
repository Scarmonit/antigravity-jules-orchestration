...
// Rate limiting - Robust Redis-based implementation with failover
const rateLimiter = new RedisRateLimiter({
  tiers: {
    free: { requestsPerMinute: 100, burstCapacity: 150, windowMs: 60000 },
    pro: { requestsPerMinute: 1000, burstCapacity: 1500, windowMs: 60000 }
  },
  failover: {
...
// Initialize rate limiter asynchronously but start using it immediately (failover handles startup)
rateLimiter.initialize().catch(err => {
  console.error('[RateLimiter] Initialization error:', err.message);
});
...
app.get('/', (req, res) => {
  res.json({
    ...
    capabilities: ['sessions', 'tasks', 'orchestration', 'mcp-protocol', 'sources', 'batch', 'monitor', 'github', 'qwen', 'system_stats'],
    authMethod: 'api-key',
    endpoints: {
      health: '/health',
      tools: '/mcp/tools',
...
app.get('/mcp/tools', cacheMiddleware, (req, res) => {
  res.json({
    tools: [
      ...
      // NEW: System Stats
      { name: 'jules_system_stats', description: 'Get system metrics (memory, uptime)', parameters: {} }
    ]
  });
});
...
function initializeToolRegistry() {
  ...
  // NEW: System Stats
  toolRegistry.set('jules_system_stats', () => ({
    uptime: process.uptime(),
